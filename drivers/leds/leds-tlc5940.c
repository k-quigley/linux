...
tlc5940_update_fb(struct tlc5940 *const tlc)
{
	...
	for (id = 0; id < TLC5940_MAX_LEDS; id++) {
		...
		if (mid_byte) {
			// MSB 4 bits of brightnes goes into fb[offset] lower half + preserve
			fb[offset] = (fb[offset] & 0xf0) | brightness >> 8;
			// LSB 8 bits of brightnes goes into fb[offset+1]
			fb[offset + 1] = brightness & 0xff;
		} else {
			// MSB 8 bits of brightness goes into fb[offsset]
			fb[offset] = brightness >> 4;
			// LSB 4 bits of brighness goes into fb[offset] upper half + preserve
